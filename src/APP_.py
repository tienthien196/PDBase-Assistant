...
import io
import re
import threading  # Đảm bảo đã import
import hashlib  # Để tạo tên file cache

# --- CẤU HÌNH ---
CACHE_CONTEXFILE_DIR = os.path.expanduser("~/gmfiNN")  # Thư mục cache
os.makedirs(CACHE_CONTEXFILE_DIR, exist_ok=True)  # Tạo thư mục nếu chưa tồn tại

# Thử import QwenAgent
try:
...
# PROMT_A là một dict có thể thay đổi
PROMT_A = {
    ...
    "contex pdf file": ""  # Sẽ được cập nhật khi có file mới
}
...
class PDFViewerApp:
    def __init__(self, root):
        ...
        self.current_context_thread = None  # Theo dõi luồng đọc context hiện tại
    ...
    def get_cache_filename(self, file_path):
        """Tạo tên file cache từ hash của đường dẫn file PDF"""
        safe_name = hashlib.md5(file_path.encode('utf-8')).hexdigest()
        return os.path.join(CACHE_CONTEXFILE_DIR, f"{safe_name}.txt")

    def update_prompt_with_context(self, context_text):
        """Cập nhật PROMT_A với context của file hiện tại"""
        global PROMT_A
        # Luôn reset context cũ
        PROMT_A["contex pdf file"] = context_text
        print(f"✅ Context updated for prompt engineer. Length: {len(context_text)} characters.")

    def read_pdf_context_background(self):
        """Đọc toàn bộ nội dung PDF trong luồng nền và lưu cache"""
        if not self.doc or not self.current_file_path:
            return

        cache_file = self.get_cache_filename(self.current_file_path)
        ...
        if os.path.exists(cache_file):
            try:
                with open(cache_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                # Cập nhật PROMT_A trên luồng chính
                self.root.after(0, lambda: self.update_prompt_with_context(content))
                self.status.config(text=f"Cache loaded for: {os.path.basename(self.current_file_path)}")
                return
            except Exception as e:
                print(f"Warning: Cannot read cache {cache_file}: {e}")

        # Nếu không có cache, đọc từ PDF
        full_text = ""
        try:
            ...
            # Cập nhật PROMT_A
            self.root.after(0, lambda: self.update_prompt_with_context(full_text.strip()))
            self.status.config(text=f"Context cached for: {os.path.basename(self.current_file_path)}")

        except Exception as e:
            error_msg = f"Failed to read PDF context: {str(e)}"
            self.root.after(0, lambda: messagebox.showwarning("Cache Warning", error_msg))
            print(error_msg)
            # Vẫn cập nhật PROMT_A rỗng để tránh lỗi sau
            self.root.after(0, lambda: self.update_prompt_with_context(""))

    def load_pdf(self, file_path):
        try:
            # Hủy luồng đọc context cũ nếu đang chạy
            if self.current_context_thread and self.current_context_thread.is_alive():
                # Không thể kill thread trong Python, nhưng chúng ta sẽ bỏ qua kết quả nếu file thay đổi
                pass
            ...
            # Render trang đầu tiên ngay lập tức
            self.render_page()

            # Cập nhật status
            self.status.config(text=f"Opened: {os.path.basename(file_path)} | Zoom: {self.zoom_factor:.1f}x | Pages: {self.total_pages}")

            # BẮT ĐẦU LUỒNG NỀN ĐỂ ĐỌC CONTEXT
            self.current_context_thread = threading.Thread(
                target=self.read_pdf_context_background,
                daemon=True
            )
            self.current_context_thread.start()
    ...
    def analyze_with_ai(self, text, text_widget):
        ...
        def run_analysis():
            try:
                ...
                # LẤY CONTEXT HIỆN TẠI TRỰC TIẾP TỪ PROMT_A
                current_context = PROMT_A.get("contex pdf file", "")

                input_user = (
                    f"[DOCUMENT CONTEXT - REFERENCED FOR BACKGROUND]\n{current_context}\n\n"
                    f"[USER'S SELECTED TEXT FOR ANALYSIS]\n{text}\n\n"
                    f"Please analyze the selected text in detail, using the document context above for background. "
                    f"Explain clearly and non-academically."
                )

                question = {
                    "promt engineer": {k: v for k, v in PROMT_A.items() if k != "contex pdf file"}, # Gửi role/job thôi
                    "document_context_length": len(current_context),
                ...
                resp = self.qwen_agent.send(str(question), stream=True) # Chuyển dict thành string nếu cần
